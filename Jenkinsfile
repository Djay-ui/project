...
// 		}
		stage('Build') {
			steps{
				script{
				sh 'sudo docker build . -t dhananjaytupe748/project_new:${DOCKER_TAG}'
				}
			}
		}
		...
		stage('Login & Push') {
			steps{
				...
				withCredentials([string(credentialsId: 'dockerhubpass', variable: 'dockerhubpass')]) {
					sh "sudo docker login -u dhananjaytupe748 -p ${dockerhubpass}"
				}
					sh 'sudo docker push dhananjaytupe748/project_new:${DOCKER_TAG}'
				}
			}
		}
